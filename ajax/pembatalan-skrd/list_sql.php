<?php
	// if ($_POST['type_retribusi']=='2') {
		
	// }else {
	// 	$list_sql = "SELECT a.tipe_retribusi,a.kd_billing,a.nm_rekening,a.status_bayar,a.status_lunas,
	// 			b.nm_wp_wr FROM app_skrd as a 
	// 			LEFT JOIN (SELECT npwrd,nm_wp_wr FROM app_reg_wr) as b ON (a.npwrd=b.npwrd)";
	// }
	$list_sql = "SELECT a.kd_billing,a.ntpd,a.nm_rekening,a.total_retribusi,a.total_bayar,to_char(a.tgl_pembayaran,'dd-mm-yyyy') as tgl_pembayaran,
				b.nm_wp_wr,c.tipe_retribusi FROM app_pembayaran_retribusi as a 
				LEFT JOIN (SELECT npwrd,nm_wp_wr FROM app_reg_wr) as b ON (a.npwrd=b.npwrd)
				LEFT JOIN (SELECT tipe_retribusi,kd_billing FROM app_skrd) as c ON (a.kd_billing=c.kd_billing)";
?>