...
header("content-disposition: attachment;filename=lap_realisasi".date('dmY').".xls");
$kd_rekening = $_GET['rek'];
	...
	if($kd_rekening=='' and $kecamatan=='')
	{
		...
		$list_sql = "SELECT a.nm_rekening,a.kd_rekening,a.masa_awal,a.tahun_pajak,a.ntp,a.kode_billing,a.tgl_pembayaran,a.sptpd_yg_dibayar,b.* FROM payment.pembayaran_sptpd as a 
				LEFT JOIN (SELECT kd_billing,no_skrd,wp_wr_nama FROM app_skrd) as b ON (a.kode_billing=b.kd_billing)";
	}
	...
	{//"SELECT a.nm_rekening,a.kd_rekening,a.bln_retribusi,a.thn_retribusi,a.ntpd,a.total_bayar,b.* FROM app_pembayaran_retribusi as a 
	$list_sql = 
	            "SELECT a.nm_rekening,a.kd_rekening,a.masa_awal,a.tahun_pajak,a.ntp,a.kode_billing,a.tgl_pembayaran,a.sptpd_yg_dibayar,b.* FROM payment.pembayaran_sptpd as a 
...
if($list_of_data->RecordCount()>0)
  					{
	  					...
	  					while($row=$list_of_data->FetchRow())
	  					{
							  ...
							  $masa_awal1=substr($masa_awal,5,2);
							  
							 if($masa_awal1==01){$bulan='Januari';}else if ($masa_awal1==02){$bulan='Pebruari';}
							 else if ($masa_awal1==03){$bulan='Maret';}else if ($masa_awal1==04){$bulan='April';}
							  else if ($masa_awal1==05){$bulan='Mei';}else if ($masa_awal1==06){$bulan='Juni';}
							  else if ($masa_awal1==07){$bulan='Juli';} else if ($masa_awal1==08){$bulan='Agustus';}
							  else if ($masa_awal1==09){$bulan='September';}else if ($masa_awal1==10){$bulan='Oktober';}
							  else if ($masa_awal1==11){$bulan='November';}else if ($masa_awal1==12){$bulan='Desember';}
	  						...
	  						<td>".$nm_rekening."</td>
	  						<td align='center'>".$kd_rekening."</td>
	  						<td align='center'>".$bulan." ".$tahun_pajak."</td>
	  						<td align='center'>".$no_skrd."</td>
	  						<td>".$wp_wr_nama."</td>
							<td>".$wp_wr_camat."</td>
						
	  						<td align='center'>".$kode_billing."'</td>
								<td>".substr($tgl_pembayaran,0,10)."</td>
							<td align='center'>&nbsp;".$ntp."</td>
	  						<td align='right'>".number_format($sptpd_yg_dibayar)."</td>
	  						</tr>";
	  						$total_retribusi += $sptpd_yg_dibayar;
	  					}
	  					echo "
	  					<tfoot>
	  						<tr>
