	...
	<table id='data-table-jq' class='table table-striped table-bordered table-hover' width='100%'>
		...
		<tbody>";
			...
			while($row = $list_of_data->FetchRow())
			{
				...
				<td align='center'>";
					if($editAccess)
		                echo "<a href='ajax/".$fn."/form_content.php?act=edit&id=".$id."&fn=".$fn."&men_id=".$men_id."' title='Acept' class='btn btn-xs btn-default' id='edit_".$no."' data-toggle='modal' data-target='#remoteModal'>";
		            else
		            	echo "<a href='javascript:;' title='Acept' class='btn btn-xs btn-default' id='edit_".$no."' onclick=\"alert('Anda tidak memiliki hak akses untuk merubah data !');\">";
					...
					<input type='hidden' id='ajax-req-dt' name='id2' value='".$id."'/>	                
