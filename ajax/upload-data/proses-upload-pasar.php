...
$DML1 = new DML('app_skrd_pasar', $db);
$DML2 = new DML('app_nota_perhitungan_pasar', $db);
$DML3 = new DML('payment_retribusi_pasar', $db);
...
if (isset($_FILES['fileExcel']['name']) && $namaFile <> '') {
    ...
    foreach ($object->getWorksheetIterator() as $worksheet) {
        ...
        for ($row = 2; $row <= $highestRow; $row++) {
            ...
            $id_skrd = $global->get_incrementID('app_skrd_pasar', 'id_skrd');
            ...
            $alamat_wpwr = $db->getOne("SELECT wp_wr_alamat FROM app_skrd_pasar WHERE npwrd='" . $npwrd . "'");
			
			$no_skrdx = $global->get_incrementID('app_skrd_pasar', 'no_skrd');
			
			$kd_billing= $no_skrdx."".$thn_retribusi."".$bln_retribusi."".$id_skrd;
			//$kd_billing=$kd_billings+1;

            $arr_data['no_skrd'] = $no_skrdx;
            ...
            $arr_data['tgl_input'] = $tgl_skrd2;
            $arr_data['user_input'] = 'Destia';
            $arr_data['status_ketetapan'] = '1';
            $arr_data['status_bayar'] = '1';
            $arr_data['status_lunas'] = '1';
			...
			$arr_data['tgl_penetapan'] = $tgl_skrd2;
			...
			// payment pasar 
			$id_pembayaran = $global->get_incrementID('payment_retribusi_pasar', 'id_pembayaran');
			
//$sql_idP="select max(id_pembayaran)as last from payment_retribusi_pasarx";
...
//$id_pembayaran = $id_pembayarans+1;
			...
			$arr_data3['id_pembayaran'] = $id_pembayaran;
            $arr_data3['npwrd'] = $npwrd;
			$arr_data3['bln_retribusi'] = $bln_retribusi;
            $arr_data3['thn_retribusi'] = $thn_retribusi;
            $arr_data3['kd_billing'] = $kd_billing;
			$arr_data3['kd_rekening'] = $kd_rek;
            $arr_data3['nm_rekening'] = $nm_rek;
			$arr_data3['ntpd'] = $id_skrd."".$thn_retribusi."".$bln_retribusi;
            $arr_data3['pembayaran_ke'] = '1';            
            $arr_data3['total_retribusi'] = $total_retribusi;
            $arr_data3['denda'] = '0';
            $arr_data3['total_bayar'] = $total_retribusi;
           	$arr_data3['tgl_pembayaran'] = $tgl_skrd2;
            ...
            $arr_data2['no_nota_perhitungan'] = $no_skrdx;
            ...
            $sql = "SELECT no_nota_perhitungan FROM app_nota_perhitungan_pasar WHERE kd_rekening='" . $kd_rek . "' AND thn_retribusi='" . $thn_retribusi . "' 
				AND no_nota_perhitungan='" . $no_nota . "'";
            $id_nota = $db->getOne($sql);

            if (is_null($id_nota) or empty($id_nota)) {
                $id_nota = $global->get_incrementID('app_nota_perhitungan_pasar', 'id_nota');
