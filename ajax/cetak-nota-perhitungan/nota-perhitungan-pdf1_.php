  ...
  if(!is_null($row['id_nota']) and !empty($row['id_nota'])){
    $system_params = $global->get_system_params();
  	...
  	$html = "<!DOCTYPE html><html><head><meta charset='UTF-8'>
        	 ...
        	 <tr>
        	 <td width='32%' style='border-right:1px solid #000;border-bottom:1px solid #000;'>
             <table border=0 width='100%'>
             <tr><td width='20%'><img src='../../img/logo_pemkot_bekasi.jpg' width='72'/></td>
             <td valign='top'><h4>PEMERINTAH ".strtoupper($system_params[7]." ".$system_params[6])."<br />".strtoupper($system_params[2])."</h4>
             <small>".$system_params[3]."<br />Telp. ".$system_params[4].", Fax. ".$system_params[4]."</small>
             <h4 style='margin-top:2px!important;'>".strtoupper($system_params[6])."</h4></td>
             </tr>
             </table>
             </td>
             <td width='32%' align='center' style='border-right:1px solid #000;border-bottom:1px solid #000;' valign='top'>
             <h4 style='margin:0'>SKRD<br />NOTA PERRHITUNGAN RETRIBUSI DAERAH<br />
             MASA RETRIBUSI : ".strtoupper(get_monthName($row['bln_retribusi']))." &nbsp;TAHUN : ".$row['thn_retribusi']."
             </h4><table border=0 style='100%' cellpadding=0 cellspacing=0>
             <tr><td><input type='checkbox' checked>&nbsp;SKRD</td>
             <td><input type='checkbox'>&nbsp;STRD</td><td><input type='checkbox'>&nbsp;SKRDB</td>             
             </tr><tr><td><input type='checkbox'>&nbsp;SKRDT</td><td><input type='checkbox'>&nbsp;SKRDKBT</td>
             <td><input type='checkbox'>&nbsp;SKRDN</td></tr></table>
          	 </td>
          	 <td valign='top' style='border-bottom:1px solid #000;'>
             <table border=0 width='100%'>
             <tr><td>Nomor Nota Perhitungan</td>
             <td> : ".sprintf('%02d',$row['no_nota_perhitungan'])."</td></tr><tr><td>Dasar Pengenaan</td>
             <td> : ".$row['dasar_pengenaan']."</td></tr>
             <tr><td>Nomor Register SKRD</td><td> : ".sprintf('%02d',$row['no_nota_perhitungan'])."</td></tr>
             </table></td></tr><tr>
          	 <td colspan='3' style='border-bottom:1px solid #000;'>
             <table width='100%'><tr><td width='8%'>Nama</td><td>: ".$row['wp_wr_nama']."</td>
             </tr><tr><td>Alamat</td><td>: ".$row['wp_wr_alamat']."</td></tr></table></td>
        	 </tr>
        	 </table>
        	 ...
        	 <th rowspan='2' class='rborder'>JUMLAH (Rp.)</td>
        	 </tr><tr>
        	 ...
        	 <th>7 (5x6)</th><th>8</th><th>9</th><th>10</th><th class='rborder'>11 (7+8+9+10)</th></tr>
        	 </thead>
        	 <tbody>";
	    ...
	    while($row2 = $result->FetchRow())
	    {
	        ...
	        if($i==1)
	        {
	          $html .= "<tr><td align='center' rowspan='".$n_row2."' valign='top'>1</td>
	        ...
	        $html .= "<td style='font-weight:".($row2['header']=='1'?'bold':'normal')."'>".$row2['uraian']."</td>
	          ...
	          <td align='right' class='rborder' style='font-weight:".($row2['header']=='1'?'bold':'normal')."'>".number_format($row2['total'],0,',','.')."</td>
	        </tr>";
	      }

          $html .= "<tr><td colspan='9'></td><td align='right'><b>Jumlah</b></td>
          			<td align='right' class='rborder'><b>".number_format($row['total_retribusi'],0,',','.')."</b></td>
          			</tr>
          			<tr><td colspan='3' class='bborder'></td>
          			<td colspan='9' class='bborder rborder'>Jumlah dengan huruf : <b>".ucwords(NumToWords($row['total_retribusi']))." Rupiah ---- </b></td>
          			</tr></tbody></table><br />";
          $html .= "<table width='100%'>
        			...
        			".$system_params[15]."<br />
        			NIP. ".$system_params[16]."</td><td>
          			".$system_params[6].", ".indo_date_format($row['tgl_penetapan'],'longDate')."<br /><br />
          			<table width='100%' border=0>
            		<tr><td>Nama</td><td> : ".$system_params[17]."</td>
            		</tr><tr><td>Jabatan</td><td> : Pelaksana</td></tr>
            		<tr><td colspan='2'><br /></td>
            		</tr><tr><td>Tanda Tangan</td><td> : </td></tr>
          			</table></td></tr>
					</table>
					</div>
  					</body>
  	...
  	$mpdf->WriteHTML($html);
    $numb = sprintf('%04s',$row['no_nota_perhitungan']);
