<?php	
	...
	if($tipe_retribusi=='1')
	{
		$list_sql = "SELECT a.bln_retribusi,a.thn_retribusi,a.total_retribusi,b.* FROM app_nota_perhitungan as a 
					 INNER JOIN 
					 	(SELECT x.id_skrd,x.no_skrd,to_char(x.tgl_skrd,'yyyy-mm-dd') as tgl_skrd,x.wp_wr_nama,x.wp_wr_alamat,y.* FROM app_skrd as x 
					 		LEFT JOIN (SELECT ntpd,to_char(tgl_pembayaran,'yyyy-mm-dd') as tgl_pembayaran,denda,total_bayar,kd_billing FROM app_pembayaran_retribusi) as y ON (x.kd_billing=y.kd_billing)
					 	 ".$skrd_cond.") as b 
